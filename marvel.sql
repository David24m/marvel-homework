...
-- Return ONLY the name column from the 'people' table 3.Oops! Someone at
--CodeClan spelled Alice's name wrong! Change it to reflect the proper spelling
--(change 'Alise Rees' to 'Alice Rees').
  --answer)SELECT * FROM people name; UPDATE people SET name = 'Alice Rees'
  --WHERE name = 'Alise Rees';

-- Return ONLY your name from the 'people' table.
  --answer)SELECT * FROM people WHERE name = 'David Mulligan';

-- The cinema is showing 'Batman Begins', but Batman is DC, not Marvel! Delete
--the entry from the 'movies' table.
  --answer)DELETE FROM movies WHERE title = 'Batman Begins';

-- Create a new entry in the 'people' table with the name of one of the
-- instructors.
  --answer)INSERT INTO people (name) VALUES ('Harrison Booth');


-- Craig Morton has decided to hijack our movie evening, Remove him from the
-- table of people.
  --answer)DELETE FROM people WHERE name = 'Craig Morton';

-- Somehow the list of people includes someone just named 'David'. Change our
-- mystery guest's name to 'David 5'
  --answer)UPDATE people SET name = 'David 5' WHERE name = 'David';


-- The cinema has just heard that they will be holding an exclusive midnight
-- showing of 'Spiderman Homecoming'!! Create a new entry in the 'movies' table
-- to reflect this.
  --answer)INSERT INTO movies (title, year, show_time) VALUES ('Spiderman
  --Homecoming', 2017, '00:00');


-- The cinema would also like to make the Guardian's of the Galaxy movies a back
-- to back feature. Update the 'Guardians of the Galaxy' show time from 16:15
-- to 18:30
  --answer)UPDATE movies SET show_time = '16:15' WHERE title = 'Guardians of the Galaxy';


--Research how to delete multiple entries from your table in a single command.
    --answer DELETE FROM people WHERE id BETWEEN 11 AND 14;
